        ...
        <div class="services-grid">
          <div class="formats-section">
            ...
            <div class="formats-list">
              <article 
                v-for="(формат, index) in форматы" 
                :key="формат.id"
                class="format-card"
                :style="{ animationDelay: `${index * 0.15}s` }"
              >
                <div class="format-header">
                  <h4 class="format-name">{{ формат.название }}</h4>
                  <div class="format-divider"></div>
                </div>
                <p class="format-description">{{ формат.описание }}</p>
                <div class="format-duration" v-if="формат.длительность">
                  <span class="duration-label">Длительность:</span>
                  <span class="duration-value">{{ формат.длительность }}</span>
                </div>
              </article>
            </div>
          </div>
          ...
          <div class="pricing-methods-section">
            <div class="pricing-block">
              ...
              <div class="pricing-card">
                <div class="pricing-content">
                  <span class="pricing-text">{{ стоимость }}</span>
            ...
            <div class="methods-block">
              ...
              <div class="methods-list">
                <div 
                  v-for="(способ, index) in способыПроведения" 
                  :key="способ.тип"
                  class="method-item"
                  :style="{ animationDelay: `${index * 0.1 + 0.6}s` }"
                >
                  <div class="method-icon" aria-hidden="true">{{ способ.иконка }}</div>
                  <div class="method-details">
                    <h4 class="method-type">{{ способ.тип }}</h4>
                    <p class="method-platforms" v-if="способ.платформы">{{ способ.платформы }}</p>
                  </div>
                </div>
              </div>
            </div>
...
interface ServicesProps {
  форматы?: Формат[]
  стоимость?: string
  способыПроведения?: СпособПроведения[]
}
...
withDefaults(defineProps<ServicesProps>(), {
  форматы: () => [
    {
      id: '1',
      название: 'Индивидуальные сессии',
      описание: 'Персональная работа для глубокого исследования личных вопросов, развития самопонимания и работы с индивидуальными запросами',
      длительность: '50 минут'
    },
    {
      id: '2',
      название: 'Парные консультации',
      описание: 'Работа с парами для улучшения взаимопонимания, решения конфликтов и укрепления отношений'
    },
    {
      id: '3',
      название: 'Разовые консультации',
      описание: 'Краткосрочная поддержка для решения конкретных вопросов или получения профессионального взгляда на ситуацию'
    }
  ],
